...
class Deepfashion(DatasetMixin, PRNGMixin):
    def __init__(self, config):
        ...
        df = pd.read_csv(os.path.join(self.root, "Anno", "list_bbox_inshop.txt"), skiprows=1, sep="\s+")
    ...
    def get_example(self, i):
        ...
        segmentation_path = os.path.join(self.root, "Anno", "segmentation", "img_highres", fname_segmentation)
        ...
        segmentation = rgb2index(segmentation) # TODO: resizing changes aspect ratio, which might not be okay
        segmentation = cv2.resize(segmentation, (self.size, self.size), interpolation=cv2.INTER_NEAREST)
