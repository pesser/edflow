...
class EvalHook(Hook):
    ...
    def after_epoch(self, epoch):
        ...
        cb_kwargs = retrieve(self.config, 'eval_pipeline/callback_kwargs',
                             default={})
...
def apply_callbacks(callbacks, root, in_data, out_data, config):
    ...
    callback_kwargs = retrieve(config, 'eval_pipeline/callback_kwargs',
                               default={})
...
def cbargs2cbdict(arglist):
    ...
    for arg in arglist:
        splits = arg.split(':')
        if len(splits) != 2:
            raise ValueError('Callbacks must be supplied via the commandline '
